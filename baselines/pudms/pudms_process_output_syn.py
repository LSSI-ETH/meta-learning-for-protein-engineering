...
for order in [1,2]:
    args.top_model = f'pudms_order_{order}'
    for a in alpha_list:
        alpha = str(a)
        ...
        for trunc_factor in truncate_factor_list:
            ...
            'train_len': trunc_factor, 'meta_len': 96, 'seed': 0,
            'meta_set': 1,'optimizer': args.opt_id, 'learn_rate': args.learn_rate, 
            'vnet_lr': args.vnet_lr, 'dropout': args.dropout,
            'truncate_factor': trunc_factor, 'mlc_k_grad_steps': args.mlc_k_steps, 'alpha': alpha}
            
            preds = pd.read_csv(data_path + f'test_preds_alpha_{alpha}_trunc_{str(trunc_factor)}_order_{order}.csv')
