...
train_test_LD_split_threshold = 5
...
def class_bal_df(df, truncation_multiple_factor = 2):
    ...
    if len(double_neg) > truncate_value:        
    ...
    if len(v_pos_a_neg) > truncate_value:
    ...
    if len(v_neg_a_pos) > truncate_value:
...
data_path = '../../data/5a12/'
data_type = '5a12_2ag'
test_df.to_csv(data_path + f'{data_type}_test.csv',index=False,header=True)
train_df_double_classified.to_csv(data_path + f'{data_type}_clean_train.csv',index=False,header=True)
val_df.to_csv(data_path + f'{data_type}_val.csv',index=False,header=True)
train_df.to_csv(data_path + f'{data_type}_train.csv',index=False,header=True)
