...
def batch_meta_set(meta_df_full, edit_distance_thresh, val_df = None):
    '''
    ...
    for meta_size in meta_size_list:
        ...
        #include unused validation set sequences in meta set batching for edit distance = 4
        #if this is not the case, meta sets will be too small due to lack of data
        if edit_distance_thresh == 4 and meta_size == 288:
            meta_df_full = pd.concat([meta_df_full, val_df], ignore_index = True)
...
for ed_thresh in [4,5,6,7,8]:
    ...
    if ed_thresh == 4:
        #include unused validation set sequences in meta set batching for edit distance = 4
        #if this is not the case, meta sets will be too small due to lack of data
        batch_meta_set(meta_df_full = meta_full, edit_distance_thresh=ed_thresh, val_df = val_full)
    else:
