...
class L2RW(BaseModel):
    ...
    def train_step(self, train_loader,meta_loader,epoch, tensorboard_writer, batch_size):
        ...
        for batch_idx, (inputs, labels) in enumerate(train_loader):
            ...
            labels = labels.to(self.device, non_blocking=self.non_block)
            ...
            with higher.innerloop_ctx(self.model, self.optimizer) as (meta_model, meta_opt):
                ...
                meta_train_loss = F.cross_entropy(meta_train_outputs, labels, 
                                                  reduction = 'none')
                eps = torch.zeros(meta_train_loss.size(), 
                ...
                meta_inputs = meta_inputs.to(self.device, non_blocking=self.non_block) 
                meta_labels = meta_labels.to(self.device, non_blocking=self.non_block)
